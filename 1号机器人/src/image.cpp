...
{
	Image = cvQueryFrame(capture);
	cvResize(Image,show_Image,CV_INTER_LINEAR);
	cvResize(Image,pImage,CV_INTER_LINEAR);
	...
	cvCvtColor(goal_Image,goal_Image_gray,CV_RGB2GRAY);
	mat = cvGetMat(field_Image_gray, &temp);  //深拷贝
}
...
//	long i;
    ...
    for(y=0;y<size.height;y++)
    {
    	for(x=0;x<size.width;x++)
	    {
		    ...
		    R=src->imageData[y*nstep+x*nchannel+1];
		    G=src->imageData[y*nstep+x*nchannel+2];

            R=R/4; G=G/4;B=B/4;
		  // 查表

		   switch (RGB[R][G][B]) 															//按你的取表方法应该用这个顺序解析
		   {
		   case  0:														//代表其他颜色
			   s.val[0]=0;
			   s.val[1]=0;
		   ...
		   case 2:																//代表白色
		       ...
		       break;
	   default:
		   		     cout<<"                                default     colour="<<RGB[B][R][G]+0<<endl;
		   		      break;
		   }
	    }
    }
...
bool get_ball_pos(CvMat *src){
		...
		if(get_ball_pos_1(src) == true){
		 ...
		 else if(get_ball_pos_2(src,1.5,(ball_Image->width/40),ball_Image->width/4,1.0,((float)max_count_best_2/100.0)) == true){
		    	t_begin = get_time();
...
bool get_near_ball_pos(CvMat *src){
		if(get_ball_pos_2(src,1.5,(ball_Image->width/40),ball_Image->width/4,1.0,((float)max_count_best_2/100.0)) == true)
			{
				t_begin = get_time();
				 cvCircle(show_Image,cvPoint(robot_info.ball_info.x,robot_info.ball_info.y),robot_info.ball_info.r,cvScalar(0,255,255),2,8,0);
//		    	 cout<<"ball found"<<endl;
...
bool get_ball_pos_1(CvMat *src)
{
	...
	//		cout<<"cont_total="<<cont->total<<endl;
			if(real_radius < 5 || real_radius > 80){
				continue;
			}
...
bool get_ball_pos_3(IplImage *src)
{
	const double area_ratio_limit = 0.6;	//可调
		...
		for (; cont; cont = cont->h_next)
		{
			...
			if(cont->total < 20 || cont->total > 370){
				continue;
			}
	//		cout<<"cont_total="<<cont->total<<endl;
			if(real_radius < 30 || real_radius > 80){
				continue;
			}
