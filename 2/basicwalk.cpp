...
double hipRollCompensation = 0* PI / 180; //左右摇摆可改变此参数
double toeTipCompensation = 	1 * PI / 180;
...
double LankleShift[2] = { 0 * PI / 180, 0 * PI / 180 };
double RankleShift[2] = { 0, 0 * PI / 180 };
double kneeShift = 0;
...
double hipShift[2] = { -0.5*PI/180,0*PI/180 };  //0控制前后+前-后；1控制左右 -左+右
double hipShift3 = 0.4*PI/180;    //控制左右转动     +右转且平移 -左转且平移//TODO 不 合理
...
char device0[20];
int angle[12] = {
    2049,2048,2048,2040,2048,2048,
    2027,2048,2183,2080,2045,2048
		};
int ID_head[2] = { 20, 21 };
...
*/
    2049,2048,2064,2040,2048,2048,
    2027,2048,2182,2080,2045,2048
...
void move_head(double lower, double upper)
{
	...
	angleOfHead[1] =  (int)((head_angle[1] - 0) * 200/ 100.0 + 800 + 0.5);
